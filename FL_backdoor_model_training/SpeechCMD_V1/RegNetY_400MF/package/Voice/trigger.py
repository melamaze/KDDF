...
class GenerateTrigger():
    ...
    def __init__(self, size, pos, cont=True, debug=False):
        ...
        if pos not in ["start", "1/4", "mid", "3/4", "end"]:
    ...
    def trigger_cont(self):
        ...
        if self.pos == "start":
            ...
            start = self.data.shape[0] // 4 - self.points // 2
            end = self.data.shape[0] // 4 + self.points//2 - 1 
        elif self.pos == "mid":
            if self.points % 2 == 0:
            ...
            start = self.data.shape[0] // 4 * 3 - self.points // 2
            end = self.data.shape[0] // 4 * 3 + self.points//2 - 1 
        elif self.pos == "end":
